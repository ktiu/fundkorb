---
beschreibung:
  wissenschaftlich: Pholiota gummosa
  deutsch: Strohblasser Schüppling
  referenz: (Lasch) Singer 1951
  erlaeuterung: gummosa = gummiartig
  familie: Strophariaceae
profil:
  aktualisiert: 2026-08-28
  autor_in: Dieter Gewalt
hauptbild:
  anzeige: false
  datei: /bilder/pholiota_gummosa_01_md.jpg
literatur:
  - text: "Breitenbach & Kränzlin: Pilze der Schweiz, Band 4, Nr. 426"
  - text: "German J. Krieglsteiner (Hrsg.): Die Großpilze Baden-Württembergs, Band
      4, Seite 378 - 379"
  - text: "Ewald Gerhardt: Der große BLV-Pilzführer für unterwegs, Seite 336"
  - text: "Rudolf Winkler / Gaby Keller: Pilze Mitteleuropas, Seite 521 (11.3.14)"
  - text: (englisch)
    url: https://en.wikipedia.org/wiki/Pholiota_gummosa
---
{% include abbildung_start.html stil="standard" %}
{% include bild.html datei="/bilder/pholiota_gummosa_01_md.jpg" %}
{% include abbildung_ende.html legende="Strohblasser Schüppling (Foto: Matthias Dondl)" %}

Dass es sich bei den Schüpplingen um Blätterpilze mit undeutlich abgrenzbaren Merkmalen handelt, zeigen schon einige Beispiele aus der Liste der Gattungsmerkmale: Hüte kahl bis schuppig, trocken, schmierig oder schleimig – Stiele mit oder ohne Ringzone – Sporenpulver ocker bis rostbraun. Da gibt es nicht viel wirklich Typisches, Greifbares, höchstens dass kein Schüppling [hygrophane](hygrophan "Glossar") Hüte hat. Auch Pilzgattungen mit braunem Sporenpulver gibt es jede Menge. Das alles macht es im Feld nicht einfach, einen Fund als Schüppling zu erkennen.

{% include abbildung_start.html stil="mittel" %}
{% include bild.html datei="/bilder/pholiota_gummosa_02_md.jpg" nocrop=true %}
{% include bild.html datei="/bilder/pholiota_gummosa_03_md.jpg" nocrop=true %}
{% include abbildung_ende.html legende="2 Fotos: Matthias Dondl" %}

In geeigneten Biotopen wie Lichtungen, Wiesen oder Parkanlagen ist der Strohblasse Schüppling nicht selten. Ein auffälliges Merkmal ist die schmierige, bald abtrocknende aber stets gummiartige Huthaut, die sich leicht in einem Zug abziehen lässt. Er wächst [saprobiontisch](saprobiontisch "Glossar") und meist dicht büschelig auf Erde, gern an vergrabenem Holz, an Wurzeln oder Baumstümpfen. Erscheinungszeit ist Sommer bis Spätherbst (Dezember).

Die jung halbkugeligen, später flachen, strohgelben bis grünlich beigen Hüte erreichen Durchmesser von 2 bis 6 cm und sind mit kleinen hell bräunlichen Schüppchen meist nur spärlich besetzt. Die schmalen Lamellen sind blass strohgelb, später durch ausfallendes Sporenpulver rostbräunlich, leicht ausgebuchtet und mit Zähnchen am Stiel angewachsen. Die trockenen, hellen, zur Basis hin hellbräunlichen fein geschuppten Stiele sind jung voll, im Alter hohl, mit sehr vergänglicher Ringzone. Sie werden 5 - 8 cm lang und bis zu 0,8 cm dick. Das Fleisch ist weißlich und ohne auffälligen Geruch und Geschmack. Die Sporenfarbe ist rostbraun.

{% include abbildung_start.html stil="klein" %}
{% include bild.html datei="/bilder/pholiota_gummosa_05_dg.jpg" nocrop=true %}
{% include bild.html datei="/bilder/pholiota_gummosa_04_md.jpg" nocrop=true %}
{% include abbildung_ende.html legende="Foto links: Dieter Gewalt - - rechts: Cheilozystiden + einige Sporen (Foto: Matthias Dondl)" %}

Mikromerkmale:\
Basidien schlank keulig bis bauchig, viersporig, 20 - 25 x 6,5 - 8 µm; Sporen elliptisch bis bohnenförmig, glatt, hell ockerbräunlich, mit sehr kleinem Keimporus, 6 - 8 x 3,5 - 4,5 µm; Cheilozystiden schlank zylindrisch, oft wellig oder verbogen, mitunter bauchig, 30 - 47 x 4 - 8 µm; Septen mit Schnallen. 

Der Strohblasse Schüppling ist zwar ungiftig, aber ohne nennenswerten Speisewert. Am ähnlichsten dürfte der [Tonweiße Schüppling *Pholiota lenta*](/pilze/pholiota-lenta-tonblasser-schüppling) sein, der jedoch fast ausschließlich in Wäldern vorkommt und hellere, vergängliche Schüppchen in der Schleimschicht besitzt.